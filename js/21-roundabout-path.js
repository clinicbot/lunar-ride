...
/* ==========================================================================\n   21. Roundabout rider path + minimap\n   --------------------------------------------------------------------------\n   Consume the exact arm/circle geometry produced by js/20. The route-choice\n   point (main s == J) is the scenic arm on the circle, so existing TURN logic\n   remains valid while straight riding follows the rest of the circle.\n   ========================================================================== */
(function(){
  ...
  const arcAt=(r,a0,a1,dir,t)=>{
    const d=arcDist(a0,a1,dir),a=a0+dir*d*clamp(t,0,1);return circle(r,a);
  };
  function combo(conn,connForward,r,a0,a1,dir,u){
    const cp=connForward?conn:conn.slice().reverse(),Lc=polyLen(cp),La=arcDist(a0,a1,dir)*r.R,T=Lc+La,q=clamp(u,0,1)*T;
    if(q<=Lc)return polyAt(cp,q/(Lc||1));
    return arcAt(r,a0,a1,dir,(q-Lc)/(La||1));
  }
  function comboArcConn(r,a0,a1,dir,conn,connForward,u){
    const cp=connForward?conn:conn.slice().reverse(),La=arcDist(a0,a1,dir)*r.R,Lc=polyLen(cp),T=La+Lc,q=clamp(u,0,1)*T;
    if(q<=La)return arcAt(r,a0,a1,dir,q/(La||1));
    return polyAt(cp,(q-La)/(Lc||1));
  }
  ...
  function centre(seg,s){
    if(world&&world.roundabouts){
      if(seg==='m')for(const r of world.roundabouts){
        ...
        if(d>=-r.span&&d<=0)return combo(r.arms.prev.points,true,r,r.prevAng,r.cutAng,r.dir,(d+r.span)/r.span);
        if(d>0&&d<=r.span)return comboArcConn(r,r.cutAng,r.nextAng,r.dir,r.arms.next.points,false,d/r.span);
      }
