...
const flower={count:4,pos:new Float32Array(12),nrm:new Float32Array(12),
  col:new Float32Array([.18,.72,.18, .88,.28,.82, .16,.66,.20, .92,.54,.34])};
const mush={count:9},sentinel={count:3};
const actors=Array.from({length:7},(_,i)=>({type:'bear',px:i,py:0,pz:0,k:1}));
actors.push({gcre:'cat',k:1},{gcre:'dfly',k:1},{gcre:'stag',k:1});
const world={actors,nMain:N,lapLen:25000,rx,rz,tx,tz,meshH:(x,z)=>Math.abs(z)*.18,
...
const ctx={console,Math,Float32Array,ROUTE_STEP:10,mulberry32,buildWorld:()=>world,__verdantPurpleCarpetPatchesV139:patches};ctx.globalThis=ctx;
...
const keys=['flower4HillsideCurrentV142','flower4HillsidePurpleV142','flower4HillsideBlueV142','flower4HillsideRedV142'];
for(const k of keys)if(!out.instNature.groups[k]||!out.instNature.groups[k].instances.length)throw new Error('missing v142 colour group '+k);
let pos=0,neg=0,maxOff=0,total=0;const cc=[];
for(const k of keys){const g=out.instNature.groups[k],n=g.instances.length/6;cc.push(n);total+=n;
  for(let i=0;i<g.instances.length;i+=6){const z=g.instances[i+3];if(z>0)pos++;if(z<0)neg++;maxOff=Math.max(maxOff,Math.abs(z));}}
...
if(Math.max(...cc)-Math.min(...cc)>4)throw new Error('four flower colours are not quarter-balanced: '+cc);
if(total!==t.carpetInstances||t.flowerColourCounts.join(',')!==cc.join(','))throw new Error('colour telemetry mismatch '+JSON.stringify(t));
if(t.flowerColourLabels.join(',')!=='current,purple,blue,red')throw new Error('colour labels wrong '+t.flowerColourLabels);
if(out.instNature.models[keys[0]]!==flower)throw new Error('current-colour flowers must reuse original model');
for(const k of keys.slice(1))if(out.instNature.models[k]===flower)throw new Error('tinted model not cloned '+k);
for(const k of keys.slice(1)){
  const c=out.instNature.models[k].col;
  if(Math.abs(c[0]-.18)>1e-6||Math.abs(c[1]-.72)>1e-6||Math.abs(c[2]-.18)>1e-6)throw new Error('green foliage was recoloured in '+k);
}
if(Math.abs(out.instNature.models[keys[1]].col[3]-flower.col[3])<.02)throw new Error('purple petal tint not applied');
if(Math.abs(out.instNature.models[keys[2]].col[5]-flower.col[5])<.02)throw new Error('blue petal tint not applied');
if(Math.abs(out.instNature.models[keys[3]].col[3]-flower.col[3])<.02)throw new Error('red petal tint not applied');
...
if(t.bearsBefore!==7||t.bearTarget!==14||t.bearsAdded!==7||t.bearsFinal!==14)throw new Error('bear x2 restore failed '+JSON.stringify(t));
if(out.actors.filter(a=>a.type==='bear'||a.gcre==='vbear').length!==14)throw new Error('final bear actor count is not 14');
if(out.actors.filter(a=>a.v142).length!==7)throw new Error('expected seven new v142 bears');

console.log('ok: v142 quarters mushrooms, blankets both green hillsides, randomly mixes four quarter-balanced flower colours, and restores 14 bears');
