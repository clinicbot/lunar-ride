...
for(const m of ['VERSION=154','REEF_STATIONS=350','GROUPS_PER_SIDE=4',
  ...
  if(!src.includes(m))throw new Error('missing v154 marker '+m);
...
function makeWorld(){
  ...
  return {nMain:n,rx,rz,ry,tx,tz,lapLen:n*4,groundAt:(x,z)=>-8+.1*Math.sin(x*.01),
    props:{sentinel:true,pos:new Float32Array([0,0,0])},actors,__aquaV153:{version:153},__aquaV152:{version:152,jellyfish:60}};
}
...
if(verdant.__aquaV154||!verdant.props.sentinel)throw new Error('non-Aqua world modified');
const w=ctx.buildWorld({id:'aqua',seed:14373},()=>{}),T=w.__aquaV154;
if(!T||T.version!==154||!T.hqCoral||!T.heroClusters||!T.reefPedestals||!T.clusteredComposition||!T.closeWallFeeling)throw new Error('v154 telemetry missing');
if(T.coralGroups!==2800||T.nearGroups!==700||T.midGroups!==1400||T.farGroups!==700)throw new Error('reef placement budget changed');
if(T.heroGroups!==280||T.heroTarget!==280||T.primaryHeroes!==140||T.secondaryHeroes!==140||T.heroClusterCount!==280)throw new Error('hero group plan wrong '+JSON.stringify(T));
if(T.heroGroups+T.mediumGroups+T.simpleGroups!==2800)throw new Error('LOD counts do not total reef budget');
if(T.pedestalGroups!==2800)throw new Error('pedestal coverage incomplete');
if(Object.keys(T.typeCounts).length!==6||Object.values(T.typeCounts).some(v=>v<100))throw new Error('coral type mix incomplete');
if(!T.proceduralSphereClustersReplaced||T.triangles<180000)throw new Error('cluster geometry too weak '+T.triangles);
if(!w.props||w.props.sentinel||w.props.pos.length<450000)throw new Error('props reef was not rebuilt strongly enough');
...
console.log('ok: Aqua v154 keeps the 2800 placement budget but turns close reef into pedestal-based overlapping hero clusters with 280 hero groups while preserving shared jellyfish, fish and non-Aqua worlds');
